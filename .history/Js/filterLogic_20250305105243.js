...
console.log("Before clearing, rows count:", tableBody.rows.length);
...
// Check if new data exists, if so, populate it
if (nearEarthObjects.length > 0) {
  // Create new rows for each asteroid
  nearEarthObjects.forEach(asteroid => {
  ...
  // Handle empty results if no asteroids are found
  tableBody.innerHTML = "<tr><td colspan='7' style='text-align: center; color: red;'>No asteroids found</td></tr>";
}

console.log("After populating, rows count:", tableBody.rows.length);
