function applyFilter() {
  ...
  fetchDataWithDefaultTimeout(url, requestOptions)
    .then((data) => {
      ...
      // Also update table headers if necessary:
      // Clone default headers (or clear and rebuild if dynamic) – here we assume default headers exist.
      // For potentially hazardous, add extra header.
      if (userOption === "getPotentiallyHazardousAsteroids") {
        // Check if header already exists to avoid duplicate addition.
        const th = document.createElement("th");
        th.id = "hazardousHeader";
        th.innerText = "Hazardous";
        th.style.backgroundColor = "#2b3d53"; // highlight header
        th.rowSpan = "2";
      ...
      if (
        ...
        th.id = "sizeHeader";
        th.innerText = "Size";
        th.style.backgroundColor = "yellow"; // highlight header
        th.rowSpan="2"
        tableHeader.appendChild(th);
      }
      ...
      nearEarthObjects.forEach((asteroid) => {
        ...
        // Build row HTML. We wrap cells that need to be highlighted with a span/style condition.
        // For "AsteroidByID", highlight the ID cell.
        const idCellStyle =
          userOption === "AsteroidByID"
            ? ' style="background-color: yellow;"'
            : "";
        // For "getClosestAsteroidsByDistance", highlight the kilometers cell.
        const kmCellStyle =
          userOption === "getClosestAsteroidsByDistance"
            ? ' style="background-color: yellow;"'
            : "";
        // For "getFastestAsteroids", highlight the speed cell.
        const speedCellStyle =
          userOption === "getFastestAsteroids"
            ? ' style="background-color: yellow;"'
            : "";
    ...
    `;
        ...
        if (userOption === "getPotentiallyHazardousAsteroids") {
          rowHTML += `<td style="background-color: yellow;">${
