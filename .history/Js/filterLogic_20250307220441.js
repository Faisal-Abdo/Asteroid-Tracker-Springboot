function applyFilter() {
  ...
  .then((data) => {
    ...
    // Clear all rows and headers before populating new data
    ...
    const defaultHeaders = [
      { id: "idHeader", text: "ID" },
      { id: "nameHeader", text: "Name" },
      { id: "closeApproachDateHeader", text: "Close Approach Date" },
      { id: "missDistanceKmHeader", text: "Miss Distance (Km)" },
      { id: "lunarDistanceHeader", text: "Lunar Distance" },
      { id: "missDistanceMileHeader", text: "Miss Distance (Miles)" },
      { id: "speedKmhHeader", text: "Speed (Km/h)" },
    ];
    ...
    // Conditionally add hazardous header
    ...
    // Conditionally add size header
    if (userOption === "getAsteroidsLargerThan" || userOption === "getLargestAsteroids") {
      const th = document.createElement("th");
      th.id = "sizeHeader";
      th.innerText = "Size";
      th.rowSpan = "2";
    ...
    nearEarthObjects.forEach((asteroid) => {
      ...
      // Conditionally add hazardous data
      if (userOption === "getPotentiallyHazardousAsteroids") {
        rowHTML += `<td style="background-color: #590e0e;">${asteroid.isHazardous ? "Yes" : "No"}</td>`;
      }

      // Conditionally add size data
      if (userOption === "getAsteroidsLargerThan" || userOption === "getLargestAsteroids") {
